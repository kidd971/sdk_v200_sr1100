...
/* **** Endpoints **** */
static sac_endpoint_t *audio_consumer;
...
static void app_audio_core_init(void)
{
    ...
    sac_endpoint_interface_t consumer_iface = {0};
    ...
    sac_facade_audio_endpoint_init(NULL, &consumer_iface);
    facade_set_audio_complete_callback(main_channel_audio_tx_complete_callback, NULL);
     ...
     *
     * +-----+    +-----------+    +----------------+    +-----+    +----------------+    +-------+
     * | SWC | -> | Unpacking | -> | Digital Volume | -> | CDC | -> | Mute on Glitch | -> | AUDIO |
     * +-----+    +-----------+    +----------------+    +-----+    +----------------+    +-------+
     */
    ...
    audio_packing_fallback_instance.packing_mode = USB_AUDIO_ENABLED ? SAC_SCALE_16BITS_24BITS :
    ...
    sac_endpoint_cfg_t consumer_cfg = {
    ...
    audio_consumer = sac_endpoint_init(NULL, "Audio EP (Consumer)", consumer_iface, consumer_cfg, &sac_status);
    ASSERT_SAC_STATUS(sac_status);
    ...
    sac_pipeline = sac_pipeline_init("SWC -> Audio", swc_producer, pipeline_cfg, audio_consumer, &sac_status);
    ASSERT_SAC_STATUS(sac_status);
