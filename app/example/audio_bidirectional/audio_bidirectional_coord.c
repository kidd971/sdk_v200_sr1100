...
static void app_audio_core_init(void)
{
     ...
     * Input(USB):  Stereo stream of 48kHz/24-bit packed depth samples.
    ...
    main_channel_compression_instance.sample_format = MAIN_CHANNEL_PRODUCER_SAC_SAMPLE_FORMAT;
     ...
     * Output:      Mono stream of 32kHz/16 bits.
     *
     * +-----+    +----------------+    +-----+
     * | SWC | -> | Digital Volume | -> | USB |
     * +-----+    +----------------+    +-----+
     *
     ***** NORMAL MODE I2S (Fallback mode 0) *****
     * Input:       Mono stream of 32kHz/16-bit depth samples is received over the air from the Node.
    ...
    back_channel_mute_on_underflow_instance.reload_value =
        sac_get_nb_packets_in_x_ms(30, BACK_CHANNEL_I2S_PAYLOAD_SIZE, BACK_CHANNEL_CHANNEL_COUNT,
                                   BACK_CHANNEL_CONSUMER_SAC_SAMPLE_FORMAT, I2S_SAMPLE_RATE_HZ);
    ...
    back_channel_sac_pipeline = sac_pipeline_init("SWC -> Audio", back_channel_swc_producer_endpoint,
                                                  back_channel_pipeline_cfg, back_channel_consumer_endpoint,
                                                  &sac_status);
    ASSERT_SAC_STATUS(sac_status);
