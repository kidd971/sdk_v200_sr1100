...
static void context_switch_callback(void)
{
    ...
    processing_time_sample_us = (double)facade_profiler_stop() / 1000;
...
static uint32_t log_advanced_stats(char *buffer, uint32_t buffer_size, volatile processing_time_stats_t *stats)
{
    ...
    string_length += snprintf(buffer + string_length, buffer_size - string_length,
                            "\r\n\t%-7s: %7.3f us, count: %lu", "Average", stats->average, stats->sum_count);
    string_length += snprintf(buffer + string_length, buffer_size - string_length,
                                "\r\n\t%-7s: %7.3f us, index: %lu", "Max", stats->max, stats->max_idx);
    string_length += snprintf(buffer + string_length, buffer_size - string_length,
                                "\r\n\t%-7s: %7.3f us, index: %lu", "Min", stats->min, stats->min_idx);
    string_length += snprintf(buffer + string_length, buffer_size - string_length,
                                "\r\n\t%-7s: %7.3f us", "Range", stats->max - stats->min);
