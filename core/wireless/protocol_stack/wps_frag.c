...
static void wps_frag_read_process(void *conn, void *arg);
...
static void wps_overrun_process(void *conn, void *arg);
...
static wps_rx_frame abort_frag_read(wps_connection_t *connection);

static void wps_frag_tx_success_callback(void *conn, void *arg);
static void wps_frag_tx_dropped_callback(void *conn, void *arg);
static void wps_frag_tx_fail_callback(void *conn, void *arg);
...
wps_rx_frame wps_frag_read(wps_connection_t *connection, uint8_t *payload, size_t max_size, wps_error_t *err)
{
    ...
    case FULL_FRAME_TRANSFER_TYPE: {
        if (max_size < (frame.size - sizeof(full_frame_t))) {
            *err = WPS_WRONG_RX_SIZE_ERROR;
            return abort_frag_read(connection);
        }
    ...
    do {
        ...
        case NON_LAST_FRAGMENT_TRANSFER_TYPE: {
            ...
            if ((payload_tail + fragment_size) > (payload + max_size)) {
                *err = WPS_WRONG_RX_SIZE_ERROR;
                return abort_frag_read(connection);
            }
...
void wps_frag_set_tx_success_callback(wps_connection_t *connection, void (*callback)(void *conn, void *parg),
 ...
 *
 *  @param[in] conn  SWC connection instance to pass to the application callback.
 *  @param[in] arg   Fragmentation connection instance.
 */
static void wps_frag_read_process(void *conn, void *arg)
{
    wps_rx_frame frame = {0};
    wps_error_t err = WPS_NO_ERROR;
    wps_connection_t *connection = (wps_connection_t *)arg;
    ...
    switch (transaction_control->transfer_type) {
    case FULL_FRAME_TRANSFER_TYPE: {
        if (connection->frag.fragment_index != 0) {
            /* Previous fragment is incomplete. Flush it before handling the current frame. */
            wps_frag_flush_last_transaction(connection);
        }
        if (connection->frag.rx_success_callback != NULL) {
            connection->frag.enqueued_count++;
            connection->frag.rx_success_callback(connection->cfg.conn, connection->frag.rx_success_parg_callback);
        }
        break;
    }
    case NON_LAST_FRAGMENT_TRANSFER_TYPE: {
        ...
        if (middle_fragment->fragment_number == connection->frag.fragment_index) {
            /* Fragment index matches the expected index. */
            if ((middle_fragment->fragment_number == 0)) {
                /* First fragment of a new transaction. */
                ...
                /* Middle fragment of the current transaction. */
                connection->frag.fragment_index++;
            } else {
                /* Fragment is not part of the current transaction. */
                wps_frag_read_process_fail(connection, middle_fragment->transaction_control.transaction_id);
            }
        } else if (middle_fragment->transaction_control.transaction_id != connection->frag.transaction_id) {
            /* Wrong index and not in current transaction. */
            wps_frag_flush_last_transaction(connection);
        } else {
            wps_frag_read_process_fail(connection, middle_fragment->transaction_control.transaction_id);
        }
    ...
    case LAST_FRAGMENT_TRANSFER_TYPE: {
        ...
        if ((last_fragment->fragment_number == connection->frag.fragment_index) &&
            (last_fragment->transaction_control.transaction_id == connection->frag.transaction_id)) {
            connection->frag.fragment_index = 0;
            if (connection->frag.rx_success_callback != NULL) {
                connection->frag.enqueued_count++;
                connection->frag.rx_success_callback(connection->cfg.conn, connection->frag.rx_success_parg_callback);
            }
        } else if (last_fragment->transaction_control.transaction_id != connection->frag.transaction_id) {
            /* Wrong index and not in current transaction. */
            wps_frag_flush_last_transaction(connection);
        } else {
...
static void wps_frag_flush_last_transaction(wps_connection_t *connection)
{
    ...
    uint8_t transaction_id_to_flush = connection->frag.transaction_id;
    wps_rx_frame frame;
    ...
    do {
        ...
        if (transaction_id_to_flush == first_fragment->transaction_control.transaction_id) {
            ...
            /* Start of the new frame, stop flushing. */
            middle_fragment_t *middle_fragment = (middle_fragment_t *)frame.payload;
...
static void wps_frag_read_process_fail(wps_connection_t *connection, uint8_t transaction_id)
{
    ...
    do {
        ...
        if ((first_fragment->transaction_control.transfer_type == FULL_FRAME_TRANSFER_TYPE) ||
            ...
            if (first_fragment->transaction_control.transfer_type == FULL_FRAME_TRANSFER_TYPE) {
                if (connection->frag.rx_success_callback != NULL) {
                    connection->frag.enqueued_count++;
                    connection->frag.rx_success_callback(connection->cfg.conn,
                                                         connection->frag.rx_success_parg_callback);
                }
    ...
    if (connection->frag.rx_fail_callback != NULL) {
        connection->frag.rx_fail_callback(connection->cfg.conn, connection->frag.rx_fail_parg_callback);
    }
}
 ...
 *  @param[in] arg   Additional argument for processing.
 */
static void wps_overrun_process(void *conn, void *arg)
{
    wps_connection_t *connection = (wps_connection_t *)arg;
    ...
    if (connection->frag.event_callback != NULL) {
        connection->frag.event_callback(connection->cfg.conn, connection->frag.event_parg_callback);
    }
 ...
 *
 *  @param[in] connection  Fragment connection instance.
 *  @param[in] err         Pointer to the error code.
...
/** @brief Abort fragmentation read and flush the remaining fragments in the transaction from the WPS RX queue.
 *
 *  @param[in] connection  Pointer to the WPS connection.
 *  @return A WPS RX frame with payload set to NULL and size set to 0.
 */
static wps_rx_frame abort_frag_read(wps_connection_t *connection)
{
    ...
    wps_read_error_flush(connection, &error);
 ...
 *  @param[in] conn  Fragmentation connection instance.
 */
static void wps_frag_tx_success_callback(void *conn, void *parg)
{
    wps_connection_t *connection = (wps_connection_t *)parg;
    uint16_t *ptr_fragment_queue = NULL;
    ...
    if (connection->frag.remaining_fragment) {
        ...
        if (connection->frag.tx_success_callback != NULL) {
            connection->frag.tx_success_callback(conn, connection->frag.tx_success_parg_callback);
        }
 ...
 *
 *  @param[in] conn  Fragmentation connection instance.
 *  @param[in] parg  Additional parameter for callback.
 */
static void wps_frag_tx_dropped_callback(void *conn, void *parg)
{
    wps_connection_t *connection = (wps_connection_t *)parg;

    if (connection->tx_drop_callback != NULL) {
        connection->tx_drop_callback(conn, connection->tx_drop_parg_callback);
    }
}
 ...
 *  @param[in] parg  Additional parameter for callback.
 */
static void wps_frag_tx_fail_callback(void *conn, void *parg)
{
    wps_connection_t *connection = (wps_connection_t *)parg;

    if (connection->tx_fail_callback != NULL) {
        connection->tx_fail_callback(conn, connection->frag.tx_fail_parg_callback);
    }
}
