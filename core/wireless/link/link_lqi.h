...
typedef struct lqi {
    ...
    /*! Received frame has an invalid data size. */
    uint32_t received_data_size_invalid_count;
    /*! Received frame has an invalid header size. */
    uint32_t received_header_size_invalid_count;
...
/** @brief Get the received invalid data size count frame.
 *
 *  @param[in] lqi  Pointer to the LQI (Link Quality Indicator) object.
 *  @return Received invalid data size count frame.
 */
static inline uint32_t link_lqi_get_received_data_size_invalid_count(lqi_t *lqi)
{
    return lqi->received_data_size_invalid_count;
}

/** @brief Get the received invalid header size count frame.
 *
 *  @param[in] lqi  Pointer to the LQI (Link Quality Indicator) object.
 *  @return Received invalid header size count frame.
 */
static inline uint32_t link_lqi_get_received_header_size_invalid_count(lqi_t *lqi)
{
    return lqi->received_header_size_invalid_count;
}
