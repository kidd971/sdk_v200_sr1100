...
/* RX stats */
    ...
    conn->stats.packet_invalid_header_size_count = wps_stats_get_phy_received_header_size_invalid_frame_count(
        conn->wps_conn_handle);
    conn->stats.packet_invalid_data_size_count = wps_stats_get_phy_received_data_size_invalid_frame_count(
        conn->wps_conn_handle);
    conn->stats.rx_timeslot_occurrence = conn->stats.packet_rejected_count +
                                         ...
                                         conn->stats.packet_invalid_header_size_count +
                                         conn->stats.packet_invalid_data_size_count +
...
/* RX stats */
    ...
    conn->stats_per_bands[channel_number].packet_invalid_header_size_count =
        wps_stats_get_phy_received_header_size_invalid_frame_count(conn->wps_conn_handle);
    conn->stats_per_bands[channel_number].packet_invalid_data_size_count =
        wps_stats_get_phy_received_data_size_invalid_frame_count(conn->wps_conn_handle);
...
int swc_connection_format_stats(const swc_connection_t *const conn, char *const buffer, uint16_t size, swc_error_t *err)
{
    ...
    *err = SWC_ERR_NONE;
    ...
    if (conn->wps_conn_handle->is_tx_connection) {
        ...
        const char *packet_invalid_data_size_count_str = "Packet Invalid Data Size";
        const char *packet_invalid_header_size_count_str = "Packet Invalid Header Size";
        ...
        string_length =
            snprintf(buffer, size,
                     ...
                     packet_invalid_data_size_count_str, conn->stats.packet_invalid_data_size_count,
                     packet_invalid_header_size_count_str, conn->stats.packet_invalid_header_size_count,
                     packet_rejected_count_str, conn->stats.packet_rejected_count, packet_overrun_count_str,
                     conn->stats.packet_overrun_count);
    }
    /* Link margin. */
    string_length += snprintf(buffer + string_length, size - string_length, "%s:\t%10" PRIu32 "\r\n",
