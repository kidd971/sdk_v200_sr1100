...
static uint8_t get_rnsi(uint32_t freq);
                                       ...
                                       rf_channel_t *spectral)
{
    ...
    spectral->rnsi = get_rnsi(target_center_freq);
...
/** @brief Get RNSI value corresponding to a given frequency.
 *
 *  @note The function selects the RNSI associated with the closest supported frequency from a predefined lookup table.
 *
 *  @param[in] freq  Frequency value for which the RNSI is requested.
 *  @return RNSI value.
 */
static uint8_t get_rnsi(uint32_t freq)
{
    /* Frequency and corresponding RNSI values (rounded to nearest integer) */
    const uint32_t freqs[] = {6708, 7227, 7718, 8184, 8714, 9179};
    const uint8_t rnsi[] = {92, 91, 92, 93, 95, 96};
    const int n = sizeof(freqs) / sizeof(freqs[0]);

    int closest_index = 0;
    uint32_t min_diff = (freq > freqs[0]) ? (freq - freqs[0]) : (freqs[0] - freq);

    /* Find the closest frequency */
    for (int i = 1; i < n; i++) {
        uint32_t diff = (freq > freqs[i]) ? (freq - freqs[i]) : (freqs[i] - freq);
