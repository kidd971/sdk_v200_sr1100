...
static void audio_init_sai(quasar_sai_config_t sai_config)
{
    ...
    if (HAL_SAI_InitProtocol(&hsai_tx, SAI_I2S_LSBJUSTIFIED, sai_config.sai_bit_depth, QUASAR_SAI_NB_OF_SLOTS) !=
        HAL_OK) {
        while (1);
    }
    if (HAL_SAI_InitProtocol(&hsai_rx, SAI_I2S_LSBJUSTIFIED, sai_config.sai_bit_depth, QUASAR_SAI_NB_OF_SLOTS) !=
        HAL_OK) {
        while (1);
    }
