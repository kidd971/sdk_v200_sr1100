...
/* PUBLIC FUNCTIONS ***********************************************************/
void quasar_clock_init(quasar_clk_freq_t quasar_clk_freq, quasar_bsp_status_t *err)
{
    ...
    *err = QUASAR_OK;
    ...
    QUASAR_BSP_CHECK_ERROR(hal_status != HAL_OK, err, QUASAR_ERR_CLOCK_INIT, return);

    /* Calculate the flash latency. */
    flash_latency = get_flash_latency(voltage_scale, quasar_clk_freq);

    config_clock_freq(&RCC_OscInitStruct, &RCC_ClkInitStruct, quasar_clk_freq, err);
    QUASAR_BSP_CHECK_ERROR(*err != QUASAR_OK, err, *err, return);

    system_clock_init(&RCC_OscInitStruct, &RCC_ClkInitStruct, flash_latency, err);
    QUASAR_BSP_CHECK_ERROR(*err != QUASAR_OK, err, *err, return);
                              ...
                              quasar_clk_freq_t quasar_clk_freq, quasar_bsp_status_t *err)
{
    ...
    case QUASAR_CLK_160MHZ:
        ...
        break;
    default:
        QUASAR_BSP_CHECK_ERROR(true, err, QUASAR_ERR_CLOCK_INIT, return);
        break;
    }
}
...
{
    /*
     * The PLL is already configured by SystemInit, so we skip its
     * configuration here by setting the PLLState to RCC_PLL_NONE.
     */
    RCC_OscInitStruct->PLL.PLLState = RCC_PLL_NONE;

    QUASAR_BSP_CHECK_ERROR(HAL_RCC_OscConfig(RCC_OscInitStruct) != HAL_OK, err, QUASAR_ERR_CLOCK_INIT, return);
    QUASAR_BSP_CHECK_ERROR(HAL_RCC_ClockConfig(RCC_ClkInitStruct, flash_latency) != HAL_OK, err, QUASAR_ERR_CLOCK_INIT,
                           return);
}
