...
/* Handle vector table for each SPI peripheral. */
SPI_HandleTypeDef *const spi_handle_table[] = {&spi_handle_spi1, &spi_handle_spi2, &spi_handle_spi3};
...
void quasar_spi_init(quasar_spi_config_t spi_config, quasar_bsp_status_t *err)
{
    *err = QUASAR_OK;
    ...
    QUASAR_BSP_CHECK_ERROR(HAL_SPI_Init(spi_handle) != HAL_OK, err, QUASAR_ERR_SPI_INIT, return);
}
...
void quasar_spi_deinit(quasar_spi_config_t spi_config, quasar_bsp_status_t *err)
{
    *err = QUASAR_OK;
    ...
    QUASAR_BSP_CHECK_ERROR(HAL_SPI_DeInit(spi_handle) != HAL_OK, err, QUASAR_ERR_SPI_DEINIT, return);
...
{
    QUASAR_SET_BIT(gpio_port_cs->BRR, 1 << gpio_pin_cs);
}

void quasar_spi_set_cs(GPIO_TypeDef *gpio_port_cs, quasar_gpio_pin_t gpio_pin_cs)
{
    QUASAR_SET_BIT(gpio_port_cs->BSRR, (1 << gpio_pin_cs));
}
                             ...
                             quasar_bsp_status_t *err)
{
    *err = QUASAR_OK;
    ...
    QUASAR_BSP_CHECK_ERROR(HAL_SPI_Init(spi_handle) != HAL_OK, err, QUASAR_ERR_SPI_BAUDRATE_CONFIG, return);
}
