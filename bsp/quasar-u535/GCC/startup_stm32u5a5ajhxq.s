...
/* ================================================================================= */
/* Optimized .data section copy routine                                              */
/* Copies initialized data from Flash to RAM using multi-word transfers.             */
/* ================================================================================= */
LoopCopyDataInit:
    ldr   r0, =_sdata         /* r0 = Destination start address (RAM) */
    ldr   r1, =_sidata        /* r1 = Source start address (Flash) */
    ldr   r2, =_edata         /* r2 = Destination end address (RAM) */

    /* Calculate the number of 8-word (32-byte) blocks to copy */
    subs  r3, r2, r0          /* r3 = Total size of .data section in bytes */
    lsrs  r3, r3, #5          /* r3 = Number of 32-byte blocks (size / 32) */
    beq   CopyDataTail        /* If size is < 32 bytes, skip to the tail copy */

CopyDataBlockLoop:
    /* Load 8 words (32 bytes) from source (Flash) into registers r4-r11 */
    /* The '!' updates the address in r1 automatically (post-increment) */
    ldmia r1!, {r4-r11}

    /* Store 8 words (32 bytes) from registers r4-r11 to destination (RAM) */
    stmia r0!, {r4-r11}

    /* Decrement block counter and loop if not zero */
    subs  r3, r3, #1
    bne   CopyDataBlockLoop

CopyDataTail:
    /* After the main block copy, handle any remaining bytes (less than 32). */
    /* We calculate the remaining words to copy. */
    ldr   r2, =_edata         /* Reload end address */
    cmp   r0, r2
    beq   DoneCopyData        /* If current address == end address, we are done. */

CopyDataWordLoop:
    /* Copy one word at a time for the remainder. */
    ldr   r3, [r1], #4        /* Load word from source and increment source pointer */
    str   r3, [r0], #4        /* Store word to destination and increment dest pointer */
    cmp   r0, r2
    bcc   CopyDataWordLoop    /* Branch if current address < end address */

DoneCopyData:
    /* Fall through to the .bss clearing routine */

/* ================================================================================= */
/* Optimized .bss section zero-fill routine                                          */
/* ================================================================================= */
LoopFillZerobss:
    ldr   r1, =_sbss          /* r1 = BSS start address */
    ldr   r2, =_ebss          /* r2 = BSS end address */
    movs  r3, #0              /* r3 = 0, the value to write */

    /* Check if there is anything to zero */
    cmp   r1, r2
    beq   ExitFillZerobss     /* If start == end, skip */

FillZerobssLoop:
    /* Store zero to the current BSS address and increment pointer */
    str   r3, [r1], #4
    cmp   r1, r2
    bcc   FillZerobssLoop     /* Branch if current address < end address */
