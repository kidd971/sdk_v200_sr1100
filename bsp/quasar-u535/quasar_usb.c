...
/* PUBLIC FUNCTIONS ***********************************************************/
void quasar_usb_init(quasar_bsp_status_t *err)
{
    *err = QUASAR_OK;
    ...
    QUASAR_BSP_CHECK_ERROR(HAL_RCCEx_PeriphCLKConfig(&PeriphClkInit) != HAL_OK, err, QUASAR_ERR_USB_PERIPH_CLK_CONFIG,
                           return);
