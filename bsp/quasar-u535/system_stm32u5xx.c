...
void SystemInit(void)
{
  ...
  MODIFY_REG(PWR->VOSR, PWR_VOSR_VOS, PWR_REGULATOR_VOLTAGE_SCALE1);
  ...
  /* Configure PLL1 */
  /* PLL source: HSE, PLLM=1, PLLN=20, PLLR=2 -> 16MHz * 20 / 2 = 160MHz */
  /* PLL1RGE=2 (16-32MHz range) required for 16MHz HSE input */
  RCC->PLL1CFGR = (3U << RCC_PLL1CFGR_PLL1SRC_Pos) | (0U << RCC_PLL1CFGR_PLL1M_Pos)
                | (2U << RCC_PLL1CFGR_PLL1RGE_Pos);
  RCC->PLL1DIVR = ((20 - 1) << RCC_PLL1DIVR_PLL1N_Pos) | (1U << RCC_PLL1DIVR_PLL1R_Pos);

  /* Enable PLL1 and PLL1R output */
  ...
  MODIFY_REG(RCC->CFGR1, RCC_CFGR1_SW, (3U << RCC_CFGR1_SW_Pos));
