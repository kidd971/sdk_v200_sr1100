...
#define QUASAR_SPI_IS_BUSY(spi_selection) \
    ((QUASAR_SPI_GET_SELECTED_HANDLE(spi_selection)->State != HAL_SPI_STATE_READY) ? true : false)
...
/* EXTERNS ********************************************************************/
extern SPI_HandleTypeDef *const spi_handle_table[];
 ...
 *
 *  @param[in] gpio_port_cs  The reference HAL GPIO port for the chip select.
